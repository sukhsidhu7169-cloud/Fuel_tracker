...
const DASHBOARD_PASSWORD = "Laadi12";
const COMPANY = "15988357 Canada Inc.";

const DRIVERS = ["Lakhwinder Singh Bhullar","Lakhwinder Singh","Jaspreet Singh","Akash Dahiya","Gagandeep Singh","Karamveer Singh","Aditya Gupta"];
const VANS    = ["White Odyssey","Black Dodge","Blue Dodge","White Transit"];
...
async function dbLoad() {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/fuel_entries?select=*&order=created_at.desc&created_at=gte.${since.toISOString()}&company=eq.${encodeURIComponent(COMPANY)}`,
  ...
  function handleFile(e) {
    ...
    reader.onload = async ev => {
      ...
      try {
        ...
        setForm(f => ({
          ...f, receipt: base64, receipt_name: file.name,
    ...
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Inter',system-ui,sans-serif",color:C.text}}>
      <div style={{background:C.card,borderBottom:`1px solid ${C.border}`,padding:"14px 18px",position:"sticky",top:0,zIndex:10}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
          ...
          <div>
            <div style={{fontSize:10,fontWeight:700,color:C.amber,textTransform:"uppercase",letterSpacing:"0.1em"}}>{COMPANY}</div>
